...
class TransTrainer():
    def __init__(self,
        ...
        if bay_opt:
            # todo fix load model to only load encoder!!
            import skopt
    ...
    def load_pretrained_model(self, json_file, weight_file):
        ...
        model = model_from_json(loaded_model, custom_objects={'CPCLayer': self.cpc})
        # load weights
        model = load_model(weight_file, custom_objects={'CPCLayer': self.cpc})

        # extract only the encoder
        model_enc = model.layers[1].layer
        ...
        x = Dense(512, activation='relu', kernel_initializer='glorot_uniform', name='b9_fc1')(model_enc.layers[-1].output)
        ...
        model_enc = Model(model_enc.input, outputs=[y1, y2, y3])
        # optimizer

        opt = Adam(lr=learning_rate_tl1, beta_1=0.9, beta_2=0.999, epsilon=1e-08, amsgrad=True, decay=decay_tl)
        model_enc.compile(loss=self.loss, optimizer=opt, metrics=['acc'])
