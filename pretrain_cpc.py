...
import pdb
import os
from batch_generator import BatchGeneratorCPC
...
class Pretrainer:
    ...
    def __init__(self, model, path_to_data, path_to_save):
        ...
        if str(model).find('model_cpc') > 0:
            ...
            self.train_gen = BatchGeneratorCPC(train_tensor, train_keys, terms=terms, positive_samples=positive_samples, predict_terms=predict_terms)
            ...
            self.valid_gen = BatchGeneratorCPC(val_tensor, val_keys, terms=terms, positive_samples=positive_samples, predict_terms=predict_terms)
        else:
    ...
    def train(self, epochs, iter_num):
        ...
        for i in range(iter_num):
            ...
            # K.clear_session()
            # build model
            model_iter = self.model.build_model(lrate=learning_rate_pre,
                                                decay=decay_pre,
                                                terms=terms,
            ...
            if os.path.exists(history_filename) == False:
                model_checkpoint = ModelCheckpoint(weights_filename, monitor='val_binary_accuracy', verbose=1, save_best_only=True, mode='max')
